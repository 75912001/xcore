...
## 错误码
### 业务错误码: [0x10000,0x1ffff]

## 消息码
### 业务消息码: [0x10000,0x1ffff]
...
- **impl**
  - **bin** 执行目录
    - [执行脚本](/impl/bin/deploy.1.gateway.1.sh) 部署脚本
    - [执行脚本](/impl/bin/start.1.gateway.1.sh) 启动脚本
  - **build**: 构建
    - **bench**: 服务基础配置
      - `gateway.bench.json` gateway 服务基础配置
      - `login.bench.json` login 服务基础配置
    - [执行脚本](/impl/build/build.sh) 构建脚本
  - **common**: 公共模块 [todo menglc]
    - **db_async**: 异步数据库
  - **protobuf**: protobuf 协议
    - [执行脚本](/impl/protobuf/gen.sh) 生成protobuf文件
  - **service**: 服务
      ...
      - 功能
        - 接受客户端的连接
          ...
          - 用户连接
            ...
            - 连接-非活跃 [todo menglc]
              - 设置非活跃时间，超时,释放用户数据 [todo menglc]
              - 非活跃期间,将发送给用户的数据,缓存到待发送队列中 [todo menglc]
            - 限制连接数 [todo menglc]
