...
const (
	...
	ServiceInfoTimeOutSec               int64  = 60                     // 信息 超时时间 秒
	AvailableLoadDefault                uint32 = 1000000                // 可用负载-默认. 1000000
	TimerScanSecondDurationDefault             = time.Millisecond * 100 // 定时器扫描间隔-默认. 100ms
	TimerScanMillisecondDurationDefault        = time.Millisecond * 25  // 定时器扫描间隔-默认. 25ms
)
